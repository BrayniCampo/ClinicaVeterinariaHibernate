...
import com.brayni.dao.ProductoDao;
...
public class ListarProductos extends JDialog {
	...
	public void llenar(List<Producto> productos) {
		...
		String cuadro [][]= new String [celdas][2];
		if(productos.size()>0) {
			
			
			for (int i=0;i<productos.size();i++) {
				cuadro[i][0]=productos.get(i).getNombreProducto();
				cuadro[i][1]=productos.get(i).getPrecioProducto()+"";
